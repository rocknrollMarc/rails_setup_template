...
* Uses {Ruby on Rails 3.2.3}[http://rubyonrails.org].
* Installs the {jQuery 1.7.2}[http://jquery.com] JavaScript library.
...
3. {Ruby on Rails 3.2.x+}[http://rubyonrails.org].
...
NOTE: These scripts are setup to support multiple environments so make sure to replace the <environment> with
the environment you are working in. For example, I like to use "stage" and "production" environments.
